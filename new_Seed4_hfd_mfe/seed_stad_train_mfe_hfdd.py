...
Using MFE (Multiscale Fuzzy Entropy) and HFD (Higuchi Fractal Dimension) Profile Losses.

Features:
- Differentiable Multiscale Fuzzy Entropy loss
- Differentiable Higuchi Fractal Dimension curve loss
- Z-score normalization for robust entropy calculation
- Combined loss: diffusion + MSE + MFE + HFD
- Default weights: MSE=0.5, HFD=0.1, MFE=0.1 (tunable)
"""
import os
import sys
import argparse
...
try:
    ...
    try:
        ...
        raise ImportError(f"Could not import HFDProfileLoss: {e} / {e2}")

from config_seed4 import Config_MAE_SEED4
from mae_for_eeg import MAEforEEG
from stad_model_CORRECT import STADModel

DEFAULT_OUTPUT_DIR = "/home/ab_students/EEG-MTP/new_Seed4_hfd_mfe"
...
def build_mfe_loss(m=2, n=2.0, tau_max=20, normalize_z=True, r_fixed=0.15, device=None):
    ...
    print(f"📊 MFE Config: m={m}, n={n}, tau_max={tau_max}, r={r_fixed}")
    return loss_fn

def build_hfd_loss(fs_hz=250.0, min_ms=4.0, max_ms=200.0, num_scales=16, device=None):
    # Generates log-spaced k samples
    k_list = k_list_logspace(fs_hz=fs_hz, min_ms=min_ms, max_ms=max_ms, num_scales=num_scales)
    loss_fn = HFDProfileLoss(k_list=k_list, distance='mse', reduction='mean')
    if device is not None:
        loss_fn = loss_fn.to(device)
    print(f"📊 HFD Config: fs={fs_hz}Hz, scales={num_scales}, range=[{min_ms}ms, {max_ms}ms]")
    return loss_fn
...
def train_stad_model(stad_model, train_loader, val_loader, args, device, output_dir):
    print("\nInitializing Complexity Losses...")
    ...
    # SEED-IV uses 250Hz sampling rate
    hfd_loss_fn = build_hfd_loss(fs_hz=250.0, min_ms=4.0, max_ms=200.0, 
                                 num_scales=args.hfd_num_scales, device=device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # TRAIN
        stad_model.train()
        train_losses = {'total': [], 'diff': [], 'mse': [], 'mfe': [], 'hfd': []}
        for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{args.epochs} [train]"):
            ...
            if device.type == 'cuda':
                with torch.amp.autocast('cuda'):
                    ...
                    # Normalization for MFE stability
                    pred_norm = (pred_sr - pred_sr.mean(dim=-1, keepdim=True)) / (pred_sr.std(dim=-1, keepdim=True) + 1e-6)
                    target_norm = (sr_eeg - sr_eeg.mean(dim=-1, keepdim=True)) / (sr_eeg.std(dim=-1, keepdim=True) + 1e-6)

                    # Compute complexity losses
                    mfe_loss = mfe_loss_fn(pred_norm.float(), target_norm.float().detach())
                    hfd_loss = hfd_loss_fn(pred_sr.float(), sr_eeg.float().detach())

                    total_loss = (diff_loss + 
                                  args.sr_loss_weight * mse_loss + 
                                  args.mfe_loss_weight * mfe_loss + 
            ...
            train_losses['hfd'].append(hfd_loss.item())

        # VALIDATE
        stad_model.eval()
        val_losses = {'total': [], 'diff': [], 'mse': [], 'mfe': [], 'hfd': []}
            ...
            for batch in tqdm(val_loader, desc=f"Epoch {epoch + 1}/{args.epochs} [val]"):
                ...
                if device.type == 'cuda':
                    with torch.amp.autocast('cuda'):
                        ...
                        vpred_norm = (vpred_sr - vpred_sr.mean(dim=-1, keepdim=True)) / (vpred_sr.std(dim=-1, keepdim=True) + 1e-6)
                        vtarget_norm = (sr_eeg - sr_eeg.mean(dim=-1, keepdim=True)) / (sr_eeg.std(dim=-1, keepdim=True) + 1e-6)
                        
                        vh_mfe_loss = mfe_loss_fn(vpred_norm.float(), vtarget_norm.float().detach())
                        vh_hfd_loss = hfd_loss_fn(vpred_sr.float(), sr_eeg.float().detach())
                        
                        vt_loss = (vd_loss + 
                                   args.sr_loss_weight * vm_loss + 
                                   args.mfe_loss_weight * vh_mfe_loss +
                                   args.hfd_loss_weight * vh_hfd_loss)
                else:
                    vd_loss, vpred_sr = stad_model(lr_eeg, hr_eeg, sr_eeg)
                    vm_loss = F.mse_loss(vpred_sr.float(), sr_eeg.float())
                    
                    vh_mfe_loss = mfe_loss_fn(vpred_sr.float(), sr_eeg.float())
                    vh_hfd_loss = hfd_loss_fn(vpred_sr.float(), sr_eeg.float())
                ...
                val_losses['mse'].append(vm_loss.item())
                val_losses['mfe'].append(vh_mfe_loss.item())
                val_losses['hfd'].append(vh_hfd_loss.item())
...
def main():
    parser = argparse.ArgumentParser('SEED-IV STAD Training — MFE + HFD + MSE Loss')
    ...
    parser.add_argument('--sr_loss_weight', type=float, default=0.5)
    parser.add_argument('--mfe_loss_weight', type=float, default=0.1)
    parser.add_argument('--hfd_loss_weight', type=float, default=0.1)
    
    # Complexity specific configs
    ...
    parser.add_argument('--hfd_num_scales', type=int, default=16)
    ...
    print(f"  Loss = diffusion + {args.sr_loss_weight} * MSE + {args.mfe_loss_weight} * MFE + {args.hfd_loss_weight} * HFD")

    print("\n" + "="*80)
    print("Training STAD (diffusion + MSE + MFE + HFD)")
    print("="*80)
