...
def reconstruct_eeg_ddim(model, x_lr, diff_params, device, steps=50, eta=0.0):
    """
    DDIM sampling for deterministic reconstruction
    
    Args:
        model: Trained STAD model
        x_lr: Low-resolution EEG (B, 16, 400)
        diff_params: Diffusion parameters
        device: cuda/cpu
        steps: Number of denoising steps (50-100 recommended)
        eta: Stochasticity parameter (0=deterministic DDIM, 1=DDPM)
    
    Returns:
        sr_eeg: Super-resolved EEG (B, 32, 400)
    """
    model.eval()
    with torch.no_grad():
        B = x_lr.shape[0]
        ...
        for i, t in enumerate(timesteps):
            ...
            # Add noise for next step (controlled by eta)
            if i < len(timesteps) - 1:
                ...
                zt = torch.sqrt(alpha_t_prev) * pred_x0 + torch.sqrt(1 - alpha_t_prev - sigma_t**2) * pred_noise + sigma_t * noise
            else:
                zt = pred_x0
        
        # Decode latent to EEG via MAE decoder
        cls_token = model.mae.cls_token.expand(B, -1, -1)
        zt_with_cls = torch.cat([cls_token, zt], dim=1)  # (B, 101, 256)
        pred_patches = model.mae.decode_full(zt_with_cls)  # (B, 100, patch_size*hr_channels)
        sr_eeg = model.mae.unpatchify(pred_patches)  # (B, 32, 400)
...
def main_reconstruct(
    ...
    with torch.no_grad():
        for batch_idx, (x_lr, y_hr) in enumerate(test_loader):
            ...
            # Reconstruct
            sr_eeg = reconstruct_eeg_ddim(model, x_lr, diff_params, device, steps=ddim_steps)
