...
class SpatioTemporalConditionNet(nn.Module):
    ...
    def forward(self, x, chan_pos, t_steps, cond_c):
        ...
        temporal_feat = temporal_feat.transpose(1, 2)  # (B, T, D)
        temporal_feat = temporal_feat.mean(dim=1, keepdim=True).expand(B, C, -1)  # (B, C, D)

        # ---- Spatial embedding ----
        # chan_pos shape: (B, n_channels, 2)
        # spatial_proj maps 2 -> latent_dim
        spatial_feat = self.spatial_proj(chan_pos)  # -> (B, n_channels, latent_dim)
