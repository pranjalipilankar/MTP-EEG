...
def compute_graph_harmonics(chan_pos, k=8):
    """Compute graph Laplacian eigenvectors for channel positions (C, 2)."""
    ...
    basis = vecs[:, 1:k+1]  # skip constant eigenvector (first)
    return torch.tensor(basis, dtype=torch.float32)


# -------------------------------------------------------------
# Spatio-Temporal Conditioning Network (Graph Harmonic)
# -------------------------------------------------------------
class GraphHarmonicConditionNet(nn.Module):
    def __init__(self, n_channels=32, model_dim=128, n_conditions=8):
        super().__init__()
        ...
        self.register_buffer("harmonics", None)  # cached basis
    ...
    def forward(self, x, chan_pos, t_steps, cond_c):
        """
        x: (B, C, T)
        chan_pos: (B, C, 2) or (C, 2)
        cond_c: unused (reserved for subject-conditioning)
        """
        B, C, T = x.shape
