...
FIXED for Patch-Based Latents (DreamDiffusion MAE)

This module denoises latent representations in the diffusion process.
Updated to work with patch-based latents (B, num_patches, dim) instead of channel-based.
"""
import torch
...
class MultiScaleTransformerDenoisingModule(nn.Module):
    """
    Multi-scale transformer for denoising patch-based latent representations.
    
    FIXED: Now works with (B, num_patches, latent_dim) instead of (B, channels, latent_dim)
    
    Args:
        num_patches: Number of patches (e.g., 100 for 400pts with patch_size=4)
        latent_dim: Latent dimension (e.g., 256 for DreamDiffusion MAE)
    ...
    def forward(self, zt, t_steps, cond_tokens, cond_pooled):
        ...
        # ✅ FIX: Initialize conditioning projection if needed (only once)
        if cond_tokens.shape[-1] != self.latent_dim:
            if self.cond_proj is None:
                print(f"⚠️  WARNING: STC embed_dim ({cond_tokens.shape[-1]}) != MTD latent_dim ({self.latent_dim})")
                print(f"   Creating projection layer. This should be fixed in model initialization!")
                self.cond_proj = nn.Linear(cond_tokens.shape[-1], self.latent_dim).to(cond_tokens.device)
                # Register as parameter so it's saved/optimized
                self.add_module('cond_proj_dynamic', self.cond_proj)
