...
# Learns log(sigma^2) for each loss term during training.
# Total = sum_i [ (1/2*sigma_i^2) * L_i + log(sigma_i) ]
# -------------------------------------------------------------
class AdaptiveLossWeights(nn.Module):
    """
    Learnable log-variance parameters for uncertainty weighting.
    One per loss term: [diff, mse, psd, pcc]
    """
    def __init__(self, n_losses=4):
        super().__init__()
        # Initialise log(sigma^2) = 0 → sigma = 1 → equal weighting at start
    ...
    def forward(self, losses):
        ...
        Returns:
            total: weighted sum with regularisation
            weights: effective weights (1 / 2*sigma^2) for logging
        """
        total   = 0.0
        weights = []
...
def combined_loss(pred_sr, sr, diff_loss, adaptive_weights):
    """
    Total loss using adaptive uncertainty weighting.
    Weights [diff, mse, psd, pcc] are learned automatically.

    Args:
        pred_sr:          (B, 62, T) predicted SR
        sr:               (B, 62, T) target SR
        diff_loss:        scalar diffusion denoising loss
        adaptive_weights: AdaptiveLossWeights module
    Returns:
        total, mse, psd, pcc_l, effective_weights
    """
    ...
    total, weights = adaptive_weights([diff_loss, mse, psd, pcc_l])
...
def train_stad_model(stad_model, train_loader, val_loader, args, device, output_dir):
    ...
    adaptive_weights = AdaptiveLossWeights(n_losses=4).to(device)

    trainable = ([p for p in stad_model.parameters() if p.requires_grad]
                 + list(adaptive_weights.parameters()))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs} [train]"):
            ...
            torch.nn.utils.clip_grad_norm_(
                ...
                + list(adaptive_weights.parameters()), 1.0
            )
        ...
        # Learned weights at end of epoch
        w = [torch.exp(-adaptive_weights.log_vars[i]).item() for i in range(4)]
        ...
        print(
            ...
            f"Weights [diff={w[0]:.3f} mse={w[1]:.3f} psd={w[2]:.3f} pcc={w[3]:.3f}]"
        )
...
def main():
    ...
    parser.add_argument('--w_psd', type=float, default=0.1,
                        help='Weight for PSD loss')
    parser.add_argument('--w_pcc', type=float, default=0.1,
                        help='Weight for PCC loss (1 - PCC)')
    ...
    print(f"Loss weights — MSE: {args.w_mse} | PSD: {args.w_psd} | PCC: {args.w_pcc}")
