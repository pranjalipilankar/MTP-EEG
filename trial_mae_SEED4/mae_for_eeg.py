...
class MAEforEEG(nn.Module):
    ...
    def patchify(self, imgs):
        """
        imgs: [N, chan, T] e.g., [32, 32, 8064]
        Output: [N, num_patches, chan*patch_size] e.g., [32, 504, 512]
        
        Each patch contains all channels for a time segment:
        - patch_size=16 means each patch covers 16 consecutive time points
        - For 32 channels, each patch = 32 channels × 16 timepoints = 512 features
        """
        N, C, T = imgs.shape
        p = self.patch_embed.patch_size
        assert T % p == 0, f"Time length {T} must be divisible by patch_size {p}"
        
        num_patches = T // p  # e.g., 8064 // 16 = 504
        
        # Reshape time dimension into patches: [N, C, T] → [N, C, num_patches, p]
        x = imgs.reshape(N, C, num_patches, p)  # [32, 32, 504, 16]
        
        # Rearrange to [N, num_patches, C, p]
        x = x.permute(0, 2, 1, 3)  # [32, 504, 32, 16]
        
        # Flatten channels and patch_size: [N, num_patches, C*p]
        x = x.reshape(N, num_patches, C * p)  # [32, 504, 512]
        
        return x

    def unpatchify(self, x):
        """
        x: [N, num_patches, chan*patch_size] e.g., [32, 504, 512]
        Output: [N, chan, T] e.g., [32, 32, 8064]
        
        Reverse of patchify: reconstruct full time series from patches
        """
        N, num_patches, _ = x.shape
        p = self.patch_embed.patch_size
        C = self.patch_embed.proj.in_channels  # Get from the conv layer
        
        # Reshape to separate channels and patch dims: [N, num_patches, C*p] → [N, num_patches, C, p]
        x = x.reshape(N, num_patches, C, p)  # [32, 504, 32, 16]
        
        # Rearrange to [N, C, num_patches, p]
        x = x.permute(0, 2, 1, 3)  # [32, 32, 504, 16]
        
        # Flatten patches back to time: [N, C, T]
        x = x.reshape(N, C, num_patches * p)  # [32, 32, 8064]
    ...
    def forward_loss(self, imgs, pred, mask):
        """
        imgs: [N, chan, T] e.g., [32, 32, 8064]
        pred: [N, num_patches, chan*patch_size] e.g., [32, 504, 512]
        ...
        # Only compute loss on masked (removed) patches
    ...
    def forward(self, imgs, img_features=None, valid_idx=None, mask_ratio=0.75):
        ...
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*c] - in patch form
        
        # Compute loss BEFORE unpatchifying
        loss = self.forward_loss(imgs, pred, mask)

        # Handle nature image loss if needed
        ...
        # DON'T unpatchify here - let the trainer do it
        # This way trainer has control over when/how to unpatchify
        # pred stays in [N, num_patches, C*p] form
        
        # Return: pred in patch form, mask indicates masked patches
