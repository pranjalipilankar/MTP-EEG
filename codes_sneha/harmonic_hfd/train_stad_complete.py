...
class STAD(nn.Module):
    ...
    def encode_hr(self, hr_eeg):
        """Encode HR to normalized latent"""
        latent = self.mae.encode_no_mask(hr_eeg)  # (B, 101, 256)
        latent = latent[:, 1:, :]  # Remove CLS → (B, 100, 256)
        
        # ✅ CRITICAL: Normalize latent to match diffusion assumptions
        # Diffusion assumes noise ~ N(0,1), so latent should too
...
def decode_latent_to_eeg(model, z_latent):
    """
    z_latent: (B, 100, 256)
    returns:  (B, 32, 400)
    """
    B = z_latent.size(0)
    cls_token = model.mae.cls_token.expand(B, -1, -1)
    z_with_cls = torch.cat([cls_token, z_latent], dim=1)
    patches = model.mae.decode_full(z_with_cls)
    eeg = model.mae.unpatchify(patches)
    return eeg
    ...
    for epoch in range(num_epochs):
        ...
        for x_lr, y_hr in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            with autocast('cuda', dtype=torch.float32):
                ...
                # Forward diffusion
                sqrt_alpha = diff_params['sqrt_alphas_cumprod'][t].view(B,1,1).clamp(min=1e-5)
                ...
                # ---------------------------------
                # ESTIMATE z0 FROM PREDICTED NOISE
                # ---------------------------------
                z0_pred = (zt - sqrt_one_minus * pred_epsilon) / sqrt_alpha

                # ---------------------------------
                # DECODE EEG
                # ---------------------------------
                sr_eeg = decode_latent_to_eeg(model, z0_pred)

                # ---------------------------------
                # HFD LOSS (safety check)
                # ---------------------------------
                Tsig = sr_eeg.shape[-1]
                max_k = torch.max(hfd_loss_fn.k_list)

                if Tsig <= max_k + 2:
                    hfd_loss = torch.tensor(0.0, device=device)
                else:
                    sr_eeg = torch.clamp(sr_eeg, -3.0, 3.0)
                    y_hr   = torch.clamp(y_hr,   -3.0, 3.0)
                    hfd_loss = hfd_loss_fn(sr_eeg, y_hr)

                # ---------------------------------
                # TOTAL LOSS
                # ---------------------------------
                loss = diff_loss + 0.25 * hfd_loss
        ...
        with torch.no_grad():
            for x_lr, y_hr in val_loader:
                ...
                lr_pos = get_channel_positions(16, device, B)
                pred_epsilon = model(x_lr, zt, t, lr_pos)
                diff_l = criterion(pred_epsilon, epsilon)
                ...
                Tsig = sr_eeg.shape[-1]
                if Tsig <= max_k + 2:
                    hfd_l = torch.tensor(0.0, device=device)
                else:
                    sr_eeg = torch.clamp(sr_eeg, -3.0, 3.0)
                    y_hr   = torch.clamp(y_hr,   -3.0, 3.0)
                    hfd_l = hfd_loss_fn(sr_eeg, y_hr)

                val_loss += (diff_l + 0.25 * hfd_l).item()
