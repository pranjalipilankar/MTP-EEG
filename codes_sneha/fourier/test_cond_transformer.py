...
def test_model(data_root="/home/ab_students/EEG-MTP/DATA/SEED",
    ...
    condition_net = SpatioTemporalConditionNet(
    ...
    print(f"✅ Loaded checkpoint '{checkpoint}' (epoch={ckpt.get('epoch','N/A')})")

    encoder.eval(); condition_net.eval(); transformer.eval(); decoder.eval()

    # ---------------------------
    # Accumulators for extended metrics
    # ---------------------------
    total_mse = total_rmse = total_mae = 0.0
    total_corr = total_r2 = total_snr = 0.0
    ...
    def compute_metrics(y_true, y_pred):
        ...
        corr = [
            np.corrcoef(y_true_np[i], y_pred_np[i])[0, 1]
        ...
        r2_vals = [
            r2_score(y_true_np[i], y_pred_np[i])
            for i in range(y_true_np.shape[0])
        ]
    ...
    with torch.no_grad():
        for inp, target, subj_id in tqdm(test_loader, desc="Testing"):
            ...
            theta = np.linspace(0, 2 * np.pi, 62, endpoint=False)
    ...
    # Save arrays
    np.savez(
        os.path.join(output_dir, f"reconstructions_{mode}.npz"),
        recons=np.concatenate(all_recons, axis=0),
        targets=np.concatenate(all_targets, axis=0),
        inputs=np.concatenate(all_inps, axis=0)
    )
    print(f"✅ Saved reconstructed EEG segments")

    # ---------------------------
    # Save per-channel CSVs + grid figure
    # ---------------------------
    if len(all_recons) > 0:
        ...
        for ch in range(n_save_channels):
            df = pd.DataFrame({
                "Input": inp_ex[ch],
                "Reconstructed": recon_ex[ch],
                "Target": target_ex[ch]
            })
            df.to_csv(os.path.join(save_dir, f"channel_{ch+1:02d}.csv"), index=False)

        print(f"✅ Saved per-channel CSVs → {save_dir}")

        fig, axes = plt.subplots(2, int(np.ceil(n_save_channels / 2)), figsize=(18, 6))
        axes = axes.flatten()
        for i in range(n_save_channels):
            ax = axes[i]
            ax.plot(target_ex[i], label="Target", linewidth=1)
            ax.plot(inp_ex[i], label="Input", linestyle="--", linewidth=0.8)
            ax.plot(recon_ex[i], label="Reconstructed", linestyle=":", linewidth=1.2)
            ax.set_title(f"Channel {i+1}")
            ax.set_xticks([]); ax.set_yticks([])
            if i == 0:
                ax.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"multi_channel_reconstruction_{mode}.png"), dpi=300)
        plt.close()
        print("📊 Saved reconstruction grid")
