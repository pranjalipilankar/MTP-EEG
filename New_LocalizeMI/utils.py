...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    omega = np.arange(embed_dim // 2, dtype=np.float32)  # Fixed: np.float -> np.float32
    omega /= embed_dim / 2.
...
def save_model(config, epoch, model, optimizer, loss_scaler, checkpoint_paths, correlation=None, is_best=False):
    ...
    if is_best:
        torch.save(to_save, os.path.join(checkpoint_paths, 'checkpoint_best.pth'))
        print(f'  -> New best model saved! (correlation: {correlation:.4f})')    

    else:
        checkpoint_filename = f'checkpoint_epoch_{epoch:04d}.pth'
        torch.save(to_save, os.path.join(checkpoint_paths, checkpoint_filename))
